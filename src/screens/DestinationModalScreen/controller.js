...
const DestinationModalScreenController = ({visible, GetOrderList, setIsDestSelecting, SetDestination, SetMarkerPosition, bookings, closeModal, from, to}) => {
    ...
    const searchDestination = () => {
        if (text.length > 2) {
            debounce(() => {
                api.request
                    .get(`https://geocode-maps.yandex.ru/1.x?apikey=aeed4c01-79da-458a-8b02-93e6b30ed33c&geocode=${text}&format=json&ll=69.279737,41.311151&spn=0.3028106689453125,0.14159780811129963`)
                    .then(res => {
    ...
    const searchOrigin = () => {
        if (origin.length > 2) {
            debounce(() => {
                api.request
                    .get(`https://geocode-maps.yandex.ru/1.x?apikey=aeed4c01-79da-458a-8b02-93e6b30ed33c&geocode=${origin}&format=json&ll=69.279737,41.311151&spn=0.3028106689453125,0.14159780811129963`)
                    .then(res => {
    ...
    const onAddressSelect = (item, isOrigin = false, recent = false) => {
        if (recent && isOrigin) {
            ...
            return SetMarkerPosition({
                longitude: parseFloat(item.routes[1].lng),
                latitude: parseFloat(item.routes[1].lat),
            });
            ...
            return SetDestination({
                ...
                coords: {
                    longitude: parseFloat(item.routes[0].lng),
                    latitude: parseFloat(item.routes[0].lat),
                }
            });
        }
